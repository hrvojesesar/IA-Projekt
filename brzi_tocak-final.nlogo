...
to setup
  ...
  setup-ulaz
  setup-izlaz
  setup-cekaonica ; dodana procedura za čekaonicu
  reset-ticks
  postavi-raspored-dolaska-klijenata
  set serviseriPostavljeni false
  set pozicijeServisera [[-10 -15] [10 0] [-10 15] [-10 0] [30 15] [10 15] [30 -15] [30 0] [10 -15]]
  set novi-serviser-tip ""
  set profit 0
  set cekaonica-red []
  set sljedeca-cekaonica-pozicija [-21 10] ; Početna pozicija za prvi klijent u čekaonici
  set aktivni-serviseri 0

  show (word "Trenutni sat: " trenutniSat "h")
end

to setup-cekaonica
  let cekaonica-start-x min-pxcor  + 5
  let cekaonica-end-x min-pxcor + 25
  let cekaonica-start-y 9
  let cekaonica-end-y 11
  set cekaonica-patches patches with [
   pxcor >= cekaonica-start-x and pxcor <= cekaonica-end-x and
   pycor >= cekaonica-start-y and pycor <= cekaonica-end-y
  ]
  ask cekaonica-patches [
    set pcolor blue
  ]
end
...
to go
  ...
  ; Dodavanje novih servisera ako je potrebno
  if count serviseri < broj-servisera [
    dodaj-nove-servisere (broj-servisera - count serviseri)
  ]
  ...
  ; Provjera svih klijenata u čekaonici da li mogu doći do slobodnog servisera
  repeat length cekaonica-red [
    let klijent-cekaonica first cekaonica-red
    if klijent-cekaonica != nobody [
      let slobodan-serviser one-of serviseri with [status = "slobodan" and member? [tipBicikla] of klijent-cekaonica vrste-bicikla]
      ifelse slobodan-serviser != nobody [
        ask klijent-cekaonica [
          set target-serviser slobodan-serviser
          move-client-to-serviser slobodan-serviser
          ask target-serviser [
            set status "zauzet"
          ]
          set naPopravku true
          set uCekaonici false
          set vrijemePopravka round (random-float 20) + 1
          show (word "Klijent " who " iz čekaonice je stigao kod servisera " [id] of target-serviser " za popravku.")
          show (word "Vrijeme popravka: " vrijemePopravka " minuta!")
        ]
        set cekaonica-red remove-item 0 cekaonica-red
      ] [
        set cekaonica-red lput klijent-cekaonica remove-item 0 cekaonica-red
      ]
    ]
  ]
...
to dodaj-nove-servisere [broj]
  let tipovi-bicikla []

  ; Add bike types from clients in the waiting queue to the list
  if not empty? cekaonica-red [
    foreach cekaonica-red [
      trenutni-klijent ->
      if not member? [tipBicikla] of trenutni-klijent tipovi-bicikla [
        set tipovi-bicikla lput [tipBicikla] of trenutni-klijent tipovi-bicikla
      ]
    ]
  ]

  ; If there are fewer than two types in the list, add random types to complete the list
  while [length tipovi-bicikla < 2] [
    set tipovi-bicikla lput one-of ["dječji" "brdski" "trkaći"] tipovi-bicikla
    set tipovi-bicikla remove-duplicates tipovi-bicikla
  ]
  ...
  while [pozicija-item = nobody and not empty? pozicije-kopija and broj > 0] [
    ...
    if pozicija-item != nobody [
      create-serviseri 1 [
        set id who
        set shape "person service"
        set color red
        set size 4
        set vrste-bicikla n-of min list 2 (length tipovi-bicikla) tipovi-bicikla
        set status "slobodan"
...
to klijent-logika
  ...
  if naPopravku = false and popravljeno = false and ideKaIzlazu = false and uCekaonici = false [
    ...
    ifelse target-serviser != nobody [
      ...
      move-client-to-serviser target-serviser
      ...
      show (word "Klijent " who " ne može pronaći slobodnog servisera ili slobodan serviser ne može popraviti njegov tip bicikla i zato ide u čekaonicu.")
      move-client-to-cekaonica
      set uCekaonici true
      set vrijemeCekanja 15
      set cekaonica-red lput self cekaonica-red
    ]
  ]
  ...
  if naPopravku = true [
    ...
    if vrijemePopravka > preostalo-vrijeme [
      show (word "Klijent " who " je prekasno došao u servis te umire.")
  ...
  if ideKaIzlazu = true [
    move-client-to-izlaz
    show (word "Klijent " who " je otišao iz servisa.")
    ;wait 1
    die
  ]

  ; Provjera čekanja u čekaonici
  if uCekaonici = true [
    set vrijemeCekanja vrijemeCekanja - 1
    if vrijemeCekanja <= 0 [
      set ideKaIzlazu true
      show (word "Klijent " who " je čekao u redu duže od 15 minuta i odlazi iz servisa.")
...
to move-client-to-cekaonica
  let target-patch patch (item 0 sljedeca-cekaonica-pozicija) (item 1 sljedeca-cekaonica-pozicija)
  while [distance target-patch > 1] [
    let step-distance min (list 1 (distance target-patch))
    let delta-x ([pxcor] of target-patch - [xcor] of self) / distance target-patch * step-distance
    let delta-y ([pycor] of target-patch - [ycor] of self) / distance target-patch * step-distance
    setxy ([xcor] of self + delta-x) ([ycor] of self + delta-y)
    wait 0.1
  ]
  setxy ([pxcor] of target-patch) ([pycor] of target-patch)
  ; Povećaj x koordinatu za sljedećeg klijenta
  set sljedeca-cekaonica-pozicija (list (item 0 sljedeca-cekaonica-pozicija - 1) (item 1 sljedeca-cekaonica-pozicija))
end

to move-client-to-serviser [serviserr]
  while [distance serviserr > 1] [
    let step-distance min (list 1 (distance serviserr))
    let delta-x ([xcor] of serviserr - [xcor] of self) / distance serviserr * step-distance
    let delta-y ([pycor] of serviserr - [ycor] of self) / distance serviserr * step-distance
    setxy ([xcor] of self + delta-x) ([ycor] of self + delta-y)
    wait 0.1
  ]
  setxy ([xcor] of serviserr) ([ycor] of serviserr)
end

to move-client-to-izlaz
  let izlaz one-of patches with [pcolor = red]
  while [distance izlaz > 1] [
    let step-distance min (list 1 (distance izlaz))
    let delta-x ([pxcor] of izlaz - [xcor] of self) / distance izlaz * step-distance
    let delta-y ([pycor] of izlaz - [ycor] of self) / distance izlaz * step-distance
    setxy ([xcor] of self + delta-x) ([ycor] of self + delta-y)
    wait 0.1
  ]
  setxy ([pxcor] of izlaz) ([pycor] of izlaz)
end
...
to setup-ulaz
  let ulaz-patchevi patches with [pxcor <= min-pxcor + 2 and abs(pycor) <= 2]
  ask ulaz-patchevi [
    set pcolor green
  ]
  set ulaz-patch one-of ulaz-patchevi
end

to setup-izlaz
  let izlaz-patchevi patches with [pxcor >= max-pxcor - 2 and abs(pycor) <= 2]
  ask izlaz-patchevi [
    set pcolor red
  ]
  set izlaz-patch one-of izlaz-patchevi
end


to setup-serviseri
  let pozicije [[-10 -15] [10 0] [-10 15] [-10 0] [30 15] [10 15] [30 -15] [30 0] [10 -15]]
...
to create-novi-klijenti
  create-klijenti 1 [
    ...
    set color white
    set size 3
    setxy (min-pxcor + 1) 0 ; Klijent dolazi točno u centar ulaza
